               ...
               int *Recessive)
{
    ...
    if (RECESSIVEALLELES) {
        RecessiveDataCheck(Geno,Recessive);
    }
        ...
        for (line = 0; line < rowsperind; line++) {
            ...
            if (PHENOTYPE) { /*-------------------------*/
                if (PHENOTYPECOL == -9) {
                    ...
                    Kill();
                } else if (PHENOTYPECOL > LABEL + POPDATA + POPFLAG + 1 +
                           EXTRACOLS) { /*too large*/
                    ...
                    phenotypecol = PHENOTYPECOL - LABEL - POPDATA - POPFLAG -1;
                }   /*figure out which
                                               column to read*/
                ...
                for (col = 0; col < EXTRACOLS + 1; col++) {
                    if (col == phenotypecol) {
                        ...
                        ReadString (intstr, ALLELLEN, INPUT);    /*skip these data*/
                    }
                }
            ...
            for (loc = 0; loc < NUMLOCI; loc++)    /*read in genotype data here*/
                for (i=0; i<colsperloc; i++) {
                    ...
                    valid = valid * CheckIfValidInt (intstr, ind, loc);
                    if (ONEROWPERIND) {
                        Geno[GenPos (ind, i, inputorder[loc])] = atoi (intstr);
                    } else {
                        Geno[GenPos (ind, line, inputorder[loc])] = atoi (intstr);
                    }
                }
 ...
 * in the datset. If it is it also terminates the program. */
{
    ...
    for (loc=0; loc<NUMLOCI; loc++) {
        ...
        if (Recessive[loc] != MISSING) {
            ...
            for (ind=0; ind<NUMINDS; ind++) {
                ...
                for (line=0; line<LINES; line++) {
                    if (Geno[GenPos (ind,line,loc)] == Recessive[loc]) {
                        rec=1;
                        recessive_shown=1;
                    } else if (Geno[GenPos (ind,line,loc)] != MISSING) {
                        dom=1;
                    }
                }
            ...
            } else if(Recessive[loc]!= NOTAMBIGUOUS) {
                recessive_allele_not_shown++;
            }
        }
    }
    if (error>100) {
        printf("Total of %d such errors\n",error);
    }
    if (error>0) {
   ...
   to the screen */
{
    ...
    if (numwords == NULL) {
        ...
        numwords[row] = word;
        for (row = numrows - 1; row >= 0;
                row--)  /*get rid of extra zeroes at end of file */
            if (numwords[row] > 0) {
                break;
            }
        numrows = row + 1;
