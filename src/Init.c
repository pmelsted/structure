 ...
 */
{
    ...
    for (ind = 0; ind < NUMINDS; ind++) {
        if (Individual[ind].PopFlag) {
            if ((Individual[ind].Population < 1)
                    || (Individual[ind].Population > MAXPOPS)) {
    ...
    if ((INFERALPHA)
            && (!(NOADMIX))) {     /*check whether alpha is needed at all */
        numnopop = 0;
        for (ind = 0; ind < NUMINDS; ind++) {
 ...
 */
{
    ...
    *sumsqlikes = 0.0;
    ...
    if (RECESSIVEALLELES) {
        ...
        CountAlleles (Geno, NumAlleles, Translation,
                      Recessive);  /*recode alleles to {0,..,1-k} */
    }
