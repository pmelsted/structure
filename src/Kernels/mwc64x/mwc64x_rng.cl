...
void MWC64X_Step(mwc64x_state_t *s)
{
    ...
    uint carry=(uint)(
                   Xn<C);                // The (Xn<C) will be zero or one for scalar
...
{
    uint2 tmp=MWC_SkipImpl_Mod64((uint2)(s->x,s->c), MWC64X_A,
                                 MWC64X_M, distance);
    s->x=tmp.x;
    s->c=tmp.y;
}

void MWC64X_SeedStreams(mwc64x_state_t *s, ulong baseOffset,
                        ulong perStreamOffset)
{
    uint2 tmp=MWC_SeedImpl_Mod64(MWC64X_A, MWC64X_M, 1, 0,
